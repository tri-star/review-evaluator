...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    github_token = secret_store.read_secret_value(arn=integrations_secret_arn, key="github/pat")
    slack_webhook = secret_store.read_secret_value(arn=integrations_secret_arn, key="slack/webhook-url")
    github_api_client = GitHubApiClient(token=github_token) if github_token else None
    github_summary_publisher = GitHubSummaryPublisher(token=github_token) if github_token else None
    ...
    daily_summary = service.build_daily_summary(repo=repo, target_date=target_date, evaluations=evaluations)
    review_store.write_summary(summary=daily_summary, period="daily", key_name=target_date)

    weekly_summary = None
    if now.weekday() == 0:
        weekly_evaluations = review_store.load_weekly_evaluations(repo=repo, end_date=now.date())
        weekly_summary = service.build_weekly_summary(
            repo=repo,
            target_date=now.date().isoformat(),
            evaluations=weekly_evaluations,
        )
        review_store.write_summary(summary=weekly_summary, period="weekly", key_name=weekly_summary["week"])
